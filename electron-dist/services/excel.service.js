...
exports.excelService = {
    ...
    async exportAppointments(filePath, filters = {}) {
        const db = (0, connection_1.getDatabase)();
        let where = 'WHERE 1=1';
        const params = [];
    ...
    async exportTransactions(filePath, filters = {}) {
        const db = (0, connection_1.getDatabase)();
        let where = 'WHERE 1=1';
        const params = [];
        if (filters.startDate) {
    ...
    async exportInvoices(filePath, filters = {}) {
        const db = (0, connection_1.getDatabase)();
        let where = 'WHERE 1=1';
        const params = [];
        if (filters.startDate) {
    ...
    async exportReport(filePath, reportData) {
        const workbook = new exceljs_1.default.Workbook();
        ...
        const transaction = db.transaction(() => {
            sheet.eachRow((row, rowNum) => {
                ...
                try {
                    ...
                    insert.run(firstName, lastName || '', phone, getValue(['phone_secondary', 'alt phone', 'phone2']) || null, getValue(['email']) || null, getValue(['date_of_birth', 'dob', 'birth date']) || null, getValue(['gender', 'sex']) || null, getValue(['address_line1', 'address', 'street']) || null, getValue(['address_line2', 'address2']) || null, getValue(['city']) || null, getValue(['state']) || null, getValue(['pincode', 'zip', 'postal code']) || null, getValue(['medical_history', 'medical history']) || null, getValue(['notes']) || null);
                    imported++;
                }
                catch (e) {
        ...
        const transaction = db.transaction(() => {
            sheet.eachRow((row, rowNum) => {
                ...
                try {
                    ...
                    insert.run(type, getValue(['category']) || null, amount, getValue(['payment_method', 'payment method', 'method']) || 'cash', date, getValue(['reference_number', 'reference', 'ref']) || null, getValue(['notes']) || null);
                    imported++;
                }
                catch (e) {
                    errors.push({ row: rowNum, error: e.message });
